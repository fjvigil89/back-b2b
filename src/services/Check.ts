...
export class CheckService {
  ...
  public async createCheck(
    ...
    const store = await getConnection(client)
      .getCustomRepository(StoreRepository)
      .findByStoreId(folio);
    if (store) {
      const prevCheck = await getConnection(client)
        .getCustomRepository(CheckRepository)
        .findLastCheck(userId);
      ...
      if (prevCheck) {
        if (type === "in") {
          ...
          if (prevCheck.dateCheckIn && prevCheck.dateCheckOut == null) {
            prevCheck.dateCheckOut = moment(new Date()).format(
              "YYYY-MM-DD HH:mm:ss",
            );
